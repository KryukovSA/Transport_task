...
void Method_potentials::solve_parallel_for_bigsize(int electric_count, double  economic_koef) {
    ...
    //cout << "Стоимость с экономическим коэффициентом: " << economic_koef << endl;
    //std::cout << std::fixed << std::setprecision(5);
    add_electric(electric_count, economic_koef); //для рандомной генерации нужен
    methodMinElem_electric();
            ...
            if (suppliersPotincials[i] == identefikator) {
                for (int j = 0; j < сonsumerPotincials.size(); j++) {
                    if (costMat[i][j].get_status() == basic && сonsumerPotincials[j] != identefikator) {
                        double temp = costMat[i][j].get_tarif() - сonsumerPotincials[j];
#pragma omp atomic write
...
void Method_potentials::redistributionSuppliesElectric() {
    ...
    while (checkOptimal() != true) {//
        ...
        if (costMat[minI][minJ].get_electric() == true || electric_make_zero > 0) {
            //cout << "Энергоэффективность уменьшиться, оптимизация прервана" << endl;
            break;
        }  
        //------------------------------------------------------------------------------------
